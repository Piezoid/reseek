...
struct FeatureProfile {
	...
	float** const m_ScoreMx;
	const vector<byte> &m_ProfRow;
};
...
static inline void SetSMx_NoRev_Row(
	...
	for (uint FeatureIdx = 0; FeatureIdx < FeatureCount; ++FeatureIdx) {
		...
		assert(ia < AlphaSize);
		const float *ScoreMxRow = prof.m_ScoreMx[ia];
	...
	uint concat_index = 0;
	const vector<byte> &ProfRowB = FeatureProfiles[0].m_ProfRow;
...
// Memory blows up with grow-only strategy due to tail of long chains
	...
	for (uint PosBBlock = 0; PosBBlock < LB; PosBBlock += BLOCK_SIZE) {
		uint AlphaCumSum = 0;
		for (uint FeatureIdx = 0; FeatureIdx < FeatureCount; ++FeatureIdx) {
			FEATURE F = Params.m_Features[FeatureIdx];
			uint AlphaSize = g_AlphaSizes2[F];
	...
	for (uint PosA = 0; PosA < LA; ++PosA)
		{
			SetSMx_NoRev_Row(Sim, ProfileA, PosA, LB, FeatureProfiles, TransposedProfileB);
		}
